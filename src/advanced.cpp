...
//data container 
//=====================
vector<std::pair<Station,int>> graph[1005];
vector<Bike> distribution;
vector<User> users;


void construct(std::string selectedCase)
{
    ...
    {
        users.push_back(User(parse_string(User_ID),s,Start_Time,End_Time,parse_string(Start_Point),parse_string(End_Point)));
    }
...
void build_graph(std::string selectedCase)
{
    ...
    while(myfile>>v>>u>>dist)
    {
        ...
        graph[a].push_back({Station(u),dist});
    }
    myfile.close(); 
}
...
void advanced(std::string selectedCase)
{
    ...
    std::cout << "start your advanced version of data structure final from here!" <<eol;
    //---------------------------
    vector<record> records;
    build_graph(selectedCase);
    construct(selectedCase);
    //----------------------------
    ...
    for(int i=0;i<=myself::max_vertex_number;i++)
    {
        ...
        {
            std::string temp=graph[i][j].first.get_ID();
            int k=parse_string(temp);
            table[i][k]=graph[i][j].second;
            table[k][i]=graph[i][j].second;
        }
    }
    ...
    for(int i=0;i<users.Size();i++)
    {
        ...
        for(int j=0;j<distribution.Size();j++)
        {
            //find rental price
            if(i==0) break;
            if(users[i].get_start_time()>=distribution[j].get_arrival_time()&& distribution[j].check() && distribution[j].Used() &&distribution[j].get_interval()==-1)
            {
                distribution[j].set_used();
            }
        }

        int source=users[i].get_start_point();
        int destination=users[i].get_end_point();
        int start_time=users[i].get_start_time();
        int end_time=users[i].get_end_time();
        int ans=start_time+table[source][destination];
        vector<std::string> request=users[i].get_bike_type();
        int time_left=end_time-ans;
        ...
        {
            auto head=request[j];
            for(int k=0;k<distribution.Size();k++) //iterate all the bike
            {
                if(distribution[k].get_bike_type()==head && 
                distribution[k].check() && distribution[k].Used()==false
                && ans<end_time && users[i].get_start_point()==distribution[k].get_source())
                {
                    distribution[k].set_arrival_time(ans);
                    distribution[k].set_interval(-1);
                    idx.push_back(distribution[k].get_bike_id());
                }
                else 
                {
                    //use free bike transfer service
                    if(distribution[k].get_bike_type()==head && distribution[k].check() && distribution[k].Used()==false)
                    {
                        int bike_transfer_time=table[distribution[k].get_source()][source];
                        int transfer_start_time=start_time;
                        int transfer_end_time=start_time+bike_transfer_time;

                        if(bike_transfer_time<time_left && start_time+bike_transfer_time+table[source][destination]<end_time)
                        {
                            distribution[k].set_used();
                            // distribution[k].set_start_time(start_time+bike_transfer_time);
                            // distribution[k].set_arrival_time(transfer_end_time+table[source][destination]);
                            distribution[k].set_interval(bike_transfer_time);
                            idx.push_back(distribution[k].get_bike_id());
                            output2<<distribution[k].get_bike_id()<<" "<<"S"<<distribution[k].get_source()<<" "<<"S"<<source<<" "<<transfer_start_time<<" "<<transfer_end_time<<" "<<"-1"<<"\n";
                            distribution[k].set_source(source);
            ...
            //todo
            //judge currcent time is greater than 
            target=-1;
            wanted_id=INT_MAX;
            for(int k=0;k<idx.Size();k++)
            {
                if(distribution[idx[k]].Used())
                    continue;
                if(distribution[idx[k]].get_rental_price()>target)
                {
                    target=distribution[idx[k]].get_rental_price();
                    wanted_id=idx[k];
                }
                else if(distribution[idx[k]].get_rental_price()==target)
                {
                    if(distribution[idx[k]].get_bike_id()<wanted_id)
                        wanted_id=idx[k];
                }
            }
        }
        ...
        int res;
        if(target!=-1 && wanted_id!=INT_MAX )
        {
            if(distribution[wanted_id].get_interval()) 
            {
                start_time=distribution[wanted_id].get_interval()+start_time;
                distribution[wanted_id].set_interval(-1); //-1 means 
                res=start_time+table[source][destination];
            }
            else 
            {
                res=ans;
            ...
            distribution[wanted_id].set_used();
            revenue=floor(distribution[wanted_id].get_rental_price()*table[source][destination]);
            distribution[wanted_id].set_arrival_time(res);
            out_bid=distribution[wanted_id].get_bike_id();
            distribution[wanted_id].set_source(destination);
            end_time=res;
            //do not
            distribution[wanted_id].update();
            AcceptOrNot=1;
        }
        else AcceptOrNot=0;
        
        if(AcceptOrNot==0)
            output0<<'U'<<users[i].get_id()<<" "<<"0"<<" "<<"0"<<" "<<"0"<<" "<<"0"<<" "<<"0"<<"\n";
        else 
        {
            //output to user_result.txt
            output0<<'U'<<users[i].get_id()<<" "<<AcceptOrNot<<" "<<out_bid<<" "<<start_time<<" "<<end_time<<" "<<revenue<<"\n";
            //output to transfer_log.txt
            output2<<out_bid<<" "<<"S"<<source<<" "<<"S"<<destination<<" "<<start_time<<" "<<end_time<<" "<<'U'<<users[i].get_id()<<"\n";
        }
    }
    int station_id;
    int bike_id;
    ...
    {
        station_id=distribution[i].get_source();
        bike_id=distribution[i].get_bike_id();
        bike_type=distribution[i].get_bike_type();
        rental_price=distribution[i].get_rental_price();
        rental_count=distribution[i].get_rental_count();
    ...
    for(int i=0;i<records.Size();i++)
    {
        ...
        records[i].get_rental_price()<<" "<<records[i].get_rental_count()<<"\n";
    }
    //---------------------
    //closing all the file
    //---------------------
